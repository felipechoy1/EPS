...
{Implementación de canales cuánticos \\ de un qubit usando Algoritmos Variacionales Cuánticos}%		% Título del trabajo de graduación
{Felipe Antonio Ixcamparic Choy}%			% autor
{Dr. Carlos Francisco Pineda Zorrilla\\y Dr. Ángel Giovanni Ramírez García}%			 % asesor
{octubre de 2025}		% mes y año de la orden de impresión
{2}							% femenino 1; masculino 2

\examenPrivado%
{M.Sc. Jorge Marcelo Ixquiac Cabrera}        % Director ECFM%
{Dra. Ana Beatriz Cosenza Muralles}          % Examinador 1%
{Dr. Juan Adolfo Ponciano}                   % Examinador 2%
{M.Sc. Edgar Anibal Cifuentes Anléu}         % Examinador 3%
{M.Sc. Freddy Estudaro Rodríguez Quezada}    % Secretario Académico%académico
...
A Carlos Pineda, por el tiempo y dedicación que tuvo conmigo en todo momento desde el inicio de mis prácticas supervisadas, así como este proyecto. Agradezco enteramente los proyectos CBF-2025-I-1548 (SECIHTI) y UNAM-PAPIIT IG101324. Haber convivido con el grupo de mecánica cuántica de la UNAM, ha sido una experiencia maravillosa. Los conocimientos, mejora de redacción y calidad de informe no habría sido sin el apoyo del grupo. 
...
\begin{longtable}{@{}p{4cm}p{11cm}@{}}  % ajusta 4cm/11cm según márgenes
\textsf{Símbolo} & \textsf{Significado}\\[8pt]
\endhead

$\mathcal{H}$ & Espacio de Hilbert o espacio de estado.\\[6pt]
$\ket{\psi}$ & Representación del vector de estado en la notación de Dirac (\textit{ket}).\\[6pt]
$\bra{\psi}$ & Vector dual al estado $\ket{\psi}$ (\textit{bra}).\\[6pt]
$\braket{\psi|\psi}$ & Producto interno entre dos vectores en la notación de Dirac (\textit{braket}).\\[6pt]
$\ketbra{\psi}{\phi}$ & Producto externo, también llamado \textit{ketbra}.\\[6pt]
$\rho$ & Matriz densidad de un sistema cuántico.\\[6pt]
$\rho_{AB}$ & Matriz densidad de un sistema compuesto A y B.\\[6pt]
$\rho_A = \text{Tr}_B(\rho_{AB})$ & Matriz densidad reducida del subsistema A (traza parcial sobre B).\\[6pt]
$\text{Tr}(\cdot)$ & Operación de traza sobre un operador.\\[6pt]
$\gamma$ & Pureza de un estado cuántico ($\gamma = \text{Tr}(\rho^2)$).\\[6pt]
$\sigma_x, \sigma_y, \sigma_z$ & Matrices de Pauli.\\[6pt]
$R_x(\theta), R_y(\theta), R_z(\theta)$ & Compuertas de rotación alrededor de los ejes X, Y y Z.\\[6pt]
$H$ & Compuerta de Hadamard.\\[6pt]
$\text{CNOT}$ & Compuerta de control-X (controlada).\\[6pt]
$\Lambda_\mathcal{E}$ & Matriz de Choi del canal cuántico $\mathcal{E}$.\\[6pt]
$\{E_k\}$ & Conjunto de operadores de Kraus para un canal cuántico.\\[6pt]
$\mathcal{E}(\rho)$ & Acción del canal cuántico $\mathcal{E}$ sobre el estado $\rho$.\\[6pt]
$p$ & Parámetro de despolarización (probabilidad de ruido).\\[6pt]
$F(\rho, \sigma)$ & Fidelidad entre dos estados cuánticos.\\[6pt]
$F_{\text{proc}}$ & Fidelidad de proceso cuántico.\\[6pt]
$U(\bm{\theta})$ & Circuito parametrizado o ansatz en un VQA.\\[6pt]
$C(\bm{\theta})$ & Función de costo en un algoritmo variacional cuántico.\\[6pt]
$\bm{\theta}$ & Vector de parámetros de un ansatz.\\[6pt]
$\text{argmin}(-C(\bm{\theta}))$ & Optimización clásica sobre los parámetros del VQA.\\
\end{longtable}
...
Para describir la dinámica de estos sistemas se utilizan los canales cuánticos, definidos como operadores completamente positivos y que preservan la traza  ~\cite{Choi2024}. La implementación de estos canales en computadoras cuánticas reales representa un desafío en la era de la tecnología cuántica de escala intermedia ruidosa (NISQ), debido a la susceptibilidad de estos dispositivos a ruido y errores. 
...
ansatz} (HEA), diseñado para replicar la acción del canal sobre un sistema
...
\end{equation}
Este estado se dice que está \textit{entrelazado}, dado que no es separable, es decir, $ \ket{\psi_{AB}} \neq \ket{\psi_A} \otimes \ket{\psi_B}$, siendo $\ket{\psi_A},\ket{\psi_B}$ estados puros que describen a los sistemas $A, B$ respectivamente. Esto es signo de correlación entre ambos subsistemas.
...
\end{figure}
La topología del sistema influye directamente en como se implementan las interacciones entre qubits dentro del circuito cuántico, especialmente cuando se hacen operaciones entre qubits que no están conectados de forma uniforme (ver figuras \ref{fig:topos_kyiv} y  \ref{fig:topos}). 
...
\label{fig:circuito8}
\end{figure}
La transpilación de circuitos viene incorporada en la biblioteca de Qiskit, es necesaria para ejecutar circuitos en computadoras cuánticas reales y será un componente clave del proyecto. Para más detalles se puede consultar la biblioteca disponible en \cite{qiskit_transpiler}.
...
\end{equation}
Por lo tanto, vemos su acción en los estados de la base:
\begin{alignat*}{2}
...
\begin{enumerate}
    ...
    \end{equation}
    \item Sea $\Lambda \geq0$ una matriz hermítica de dimensión $d \times d$. Es la representación de Choi de un de un mapeo que preserva la traza (TP) sí y solo sí 
    \begin{equation}
        \text{Tr}_2 (\Lambda) = I.
    \end{equation}
...
\begin{equation}
       ...
       F(\rho,\sigma) \equiv \text{Tr} \sqrt{\rho^{1/2} \sigma \rho^{1/2} } = \text{Tr} |\sqrt{\rho}\sqrt{\sigma}|.     \footnote{La ecuación usada aquí para describir la fidelidad es la descrita en Nielsen y Chuang. En otros textos,  como del autor original Richard Jozsa\cite{Jozsa} y Bengston \cite{bengtsson_zyczkowski_2017},  suelen llamarla  'raíz fidelidad'.}
\end{equation}  
...
lo que la estructura del \textit{ansatz} es:
 ...
 \end{equation}
donde $N$ y $M$ son el número de capas y qubits en el circuito respectivamente.
...
% \null
\appendix
\chapter*{A. Matrices de Choi reconstruidas}
\renewcommand{\thefigure}{A.\arabic{figure}}
\setcounter{figure}{0}
\begin{figure}[h!]
    \centering
    \foreach \p in {0.1, 0.2, 0.3, 0.4} {
        \begin{subfigure}{\textwidth}
            \centering
            \includegraphics[width=0.7\linewidth]{imagenes/comp_choi_p\p.png}
            \caption[Comparación de matrices de Choi para $p = \p$]{Comparación para $p = \p$}
            \label{fig:choi_p\p}
        \end{subfigure}
        \hspace{0.02\textwidth}
    }
    \caption[Comparación de matrices de Choi reconstruidas para $p = 0.1$–$0.4$]{
    Comparación de matrices de Choi objetivo y matrices reconstruidas en el simulador cuántico \texttt{AerSimulator()} y en la computadora cuántica \texttt{ibm\_kyoto}, para probabilidades de despolarización $p$ entre 0.1 y 0.4. Cada valor representa una componente real de la matriz de Choi. \textbf{Fuente:} elaboración propia.}
    \label{fig:comparacion_choi_1}
\end{figure}
\begin{figure}[h!]
    \centering
    \foreach \p in {0.5, 0.6, 0.7, 0.8} {
        \begin{subfigure}{\textwidth}
            \centering
            \includegraphics[width=0.80\linewidth]{imagenes/comp_choi_p\p.png}
            \caption[Comparación de matrices de Choi para $p = \p$]{Comparación para $p = \p$}
            \label{fig:choi_p\p}
        \end{subfigure}
        \hspace{0.02\textwidth}
    }
    \caption[Comparación de matrices de Choi reconstruidas para $p = 0.5$–$0.8$]{
    Comparación de matrices de Choi objetivo y matrices reconstruidas en el simulador cuántico \texttt{AerSimulator()} y en la computadora cuántica \texttt{ibm\_kyoto}, para probabilidades de despolarización $p$ entre 0.5 y 0.8. \textbf{Fuente:} elaboración propia.}
    \label{fig:comparacion_choi_2}
\end{figure}
\begin{figure}[h!]
    \centering
    \foreach \p in {0.9, 1.0} {
        \begin{subfigure}{\textwidth}
            \centering
            \includegraphics[width=0.80\linewidth]{imagenes/comp_choi_p\p.png}
            \caption[Comparación de matrices de Choi para $p = \p$]{Comparación para $p = \p$}
    ...
    \caption[Comparación de matrices de Choi reconstruidas para $p = 0.9$–$1.0$]{
    Comparación de matrices de Choi objetivo y matrices reconstruidas en el simulador cuántico \texttt{AerSimulator()} y en la computadora cuántica \texttt{ibm\_kyoto}, para probabilidades de despolarización $p$ entre 0.9 y 1.0. \textbf{Fuente:} elaboración propia.}
    \label{fig:comparacion_choi_3}
\end{figure}
